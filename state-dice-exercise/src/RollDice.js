...
class RollDice extends Component {
  ...
  rollDice = () => {
    ...
    let newDice = this.state.dice.map((e) => {
      ...
      return this.props.diceDict[die];
    });
      ...
      <div className="diceRoller">
        <section className="dice-layout">
          {this.state.dice.forEach((die) => {
            diceElements.push(
              <Die die={die} rolling={this.state.diceRolling}></Die>
            );
          })}
          {diceElements}
        </section>
        <section>
          <button
            ...
            disabled={this.state.diceRolling}
          >
            <span>
              {!this.state.diceRolling ? "Roll the Dice" : "Rolling..."}
            </span>
          </button>
        </section>
      </div>
