...
class Hangman extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = { nWrong: 0, guessed: new Set(), answer: randomWord(), lost: false };
  ...
  render() {
    return (
      <div className='Hangman'>
        <h1>Hangman</h1>
        <img alt = {(this.state.nWrong > 0) ? `Hangman with ${this.state.nWrong} limbs` : "Hangman setting with no man"} src={this.props.images[this.state.nWrong]} />
        <p className = "Hangman-word-wrg-gses">{`Wrong Guesses: ${this.state.nWrong}`}</p>
        <p className='Hangman-word'>{this.guessedWord()}</p>

        <div className = {(!this.state.lost ? "display-content" : "hide-content")}>
          <p className='Hangman-btns' disabled = {this.state.lost}>{this.generateButtons()}</p>
        </div>

        <div className = {(!this.state.lost ? "hide-content" : "display-content")}>
          <h3 className='Hangman-loss-msg'>Aw damn... You really just lost. Better luck next time!</h3>
        </div>

        <div className="Hangman-restart-wrap">
          <button className="Hangman-restart-btn" onClick={this.restartGame}>RESTART</button>
        </div>
      </div>
    );
  }
