...
class Hangman extends Component {
  ...
  */
  ...
  generateGameState(gameOver, winnerWinnerChickenDinner){
    if(gameOver && !winnerWinnerChickenDinner){
        return "Aw damn... You really just lost. Better luck next time!"
    }else if (winnerWinnerChickenDinner && !gameOver){
        return "You got it! You guessed the word! Good job!"
    }else if(!winnerWinnerChickenDinner && !gameOver){
        return this.generateButtons();
    }else{
         return "Wait... Something went dreadfully wrong."
    }
  }
  ...
  render() {
    ...
    // Dear christ I know this needs to be refactored...
    // const winnerWinnerChickenDinner = [...this.state.answer].filter((e) => this.state.guessed.has(e)).length === this.state.answer.length;
    // much simpler...
    const winnerWinnerChickenDinner = this.guessedWord().join("") === this.state.answer; 
    ...
    return (
      <div className="Hangman">
        ...
        <p className="Hangman-btns">
          {this.generateGameState(gameOver, winnerWinnerChickenDinner)}
        </p>

        <div className="Hangman-restart-wrap">
          <button id="Hangman-restart-btn" onClick={this.restartGame}>
            RESTART
          </button>
        </div>
      </div>
